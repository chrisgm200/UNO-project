...
class GameEngine {
    static addPlayer(state, id, name) {
        ...
        const player = { id, name, hand: [], connected: true, saidUno: false };
        return { ...state, players: [...state.players, player] };
    }
    static removePlayer(state, id) {
        return {
            ...state,
    ...
    static startGame(state) {
        ...
        const players = state.players.map((p) => {
            const { drawn, deck: newDeck } = (0, Deck_1.drawCards)(deck, [], INITIAL_HAND_SIZE);
            deck = newDeck;
            return { ...p, hand: drawn };
        });
        ...
        let firstCard;
        do {
            const { drawn, deck: newDeck } = (0, Deck_1.drawCards)(deck, [], 1);
            deck = newDeck;
            firstCard = drawn[0];
    ...
    static playCard(state, playerId, cardId, chosenColor) {
        ...
        if (!(0, Card_1.canPlay)(card, topCard, state.currentColor)) {
            throw new Error('Esa carta no se puede jugar ahora');
        }
        if ((0, Card_1.isWild)(card) && !chosenColor) {
        ...
        let newState = {
            ...
            currentColor: (0, Card_1.isWild)(card) ? chosenColor : card.color,
        };
        // Victoria
        if (newHand.length === 0) {
    ...
    static applyCardEffect(state, card) {
        let nextState = { ...state };
        switch (card.value) {
            case 'skip':
    ...
    static advanceTurn(state, steps) {
    ...
    static forceDraw(state, count) {
        const n = state.players.length;
        const targetIndex = (state.currentPlayerIndex + state.direction + n) % n;
        const { drawn, deck, discardPile } = (0, Deck_1.drawCards)(state.deck, state.discardPile, count);
        const players = [...state.players];
        players[targetIndex] = { ...players[targetIndex], hand: [...players[targetIndex].hand, ...drawn] };
        return { ...state, players, deck, discardPile };
    }
    static drawCard(state, playerId) {
        const playerIndex = state.players.findIndex((p) => p.id === playerId);
        if (playerIndex !== state.currentPlayerIndex) {
            throw new Error('No es tu turno');
        }
        const { drawn, deck, discardPile } = (0, Deck_1.drawCards)(state.deck, state.discardPile, 1);
        const players = [...state.players];
